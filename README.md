...
- **Confidence Scoring (0.0 – 1.0)**: Measures intent clarity, linguistic completeness, and semantic certainty. Clear, well-formed customer inquiries score high (≥ 0.90), while brief, ambiguous, or underspecified requests experience confidence degradation (0.70 – 0.89 for moderate ambiguity; < 0.70 for incomplete requests).
...
- **AUTONOMOUS Tier**: Safe, high-confidence (≥ 0.90) actions with zero policy flags. Resolves immediately by dispatching an automated Gmail reply and confirming resolution in Slack.
- **GUARDED Tier**: Requests falling in the moderate ambiguity band (0.70 ≤ confidence < 0.90). The action is paused and an approval request is dispatched to Slack with reasoning and confidence scores for human verification.
...
| **TC-03** | Financial Policy | Refund request within limit (₹1,500 ≤ ₹2,000) | 0.95 | `AUTONOMOUS` | `AUTONOMOUS` | **PASS** |
...
- **Gmail** — intake (IMAP read) and autonomous outbox (SMTP reply)
- **Slack** — human-approval requests, escalation alerts, and emergency fallback notifications
- **Linear** — structured incident ticket creation on the BLOCKED path
...
| File | Purpose |
|---|---|
| `pipeline.py` | Main orchestration: reads email, evaluates, routes, logs, verifies |
| `guardian.py` | Confidence/risk scoring and deterministic policy engine |
| `policy.json` | Hard-coded policy invariants (refund cap, security keywords, identity rules) |
| `eval_scenarios.py` | Benchmark suite — 12 test cases across all three tiers |
| `decisions.jsonl` | Structured audit log of every evaluation, appended per run |
| `test_gmail.py` / `test_slack.py` / `test_linear.py` | Standalone connectivity checks per integration |